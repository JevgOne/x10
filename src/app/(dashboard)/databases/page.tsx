...
import { Upload, Database, Search, Trash2, X, FileSpreadsheet, Check, AlertCircle, ChevronDown, Plus } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((db) => (
          <div key={db.id} className="glass rounded-2xl border border-border p-5 hover:border-border2 transition-all group">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-txt3">{db.source || "Import"}</p>
                </div>
              </div>
              <button onClick={() => deleteDb(db.id)} className="w-7 h-7 rounded-lg bg-surface2 flex items-center justify-center text-txt3 hover:text-red opacity-0 group-hover:opacity-100 transition-opacity">
                <Trash2 size={12} />
              </button>
            </div>
            ...
            <div className="flex items-center gap-4 text-xs text-txt2">
              <span className="flex items-center gap-1">
                <FileSpreadsheet size={12} className="text-green" />
            ...
            {db.uploadDate && (
              <div className="mt-3 pt-3 border-t border-border text-[10px] text-txt3">
                Nahrano: {new Date(db.uploadDate).toLocaleDateString("cs-CZ")}
                {db.agentName && <span className="ml-2">&middot; {db.agentName}</span>}
              </div>
            )}
          </div>
        ))}
